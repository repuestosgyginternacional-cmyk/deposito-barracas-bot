...
async function procesarMensaje(chatId, texto, nombreUsuario) {
  ...
  // VER [ubicación] — qué hay en una posición
  if (comando === 'VER' || comando === 'V') {
    if (partes.length < 2) return sendMessage(chatId, '❌ Formato: <code>VER [ubicación]</code>\nEj: <code>VER C-07-3-A</code>');
    const ubicacion = partes[1].toUpperCase();
    const productos = await supabaseGet('productos', `?ubicacion=eq.${encodeURIComponent(ubicacion)}&select=*&order=codigo`);
    if (!Array.isArray(productos) || !productos.length) {
      return sendMessage(chatId, `📍 <b>${ubicacion}</b>\n\n📭 VACÍO — No hay productos en esta posición.`);
    }
    let msg = `📍 <b>${ubicacion}</b>\n\n`;
    productos.forEach(p => {
      const estado = p.stock === 0 ? '🚨' : (p.minimo > 0 && p.stock <= p.minimo) ? '⚠️' : '✅';
      msg += `${estado} <b>${p.codigo}</b> ${p.color} — Stock: <b>${p.stock}</b>\n`;
    });
    msg += `\n<i>${productos.length} producto${productos.length > 1 ? 's' : ''} en esta posición</i>`;
    return sendMessage(chatId, msg);
  }
