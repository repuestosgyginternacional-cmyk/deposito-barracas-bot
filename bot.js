...
// ===================== NOTIFICAR A TODOS =====================
async function notificarATodos(mensaje) {
  try {
    const chats = await supabaseGet('telegram_chats', '');
    if (!Array.isArray(chats) || !chats.length) return;
    for (const chat of chats) {
      await sendMessage(chat.chat_id, mensaje);
    }
  } catch (e) {
...
async function procesarMensaje(chatId, texto, nombreUsuario) {
  ...
  if (comando === 'POSICION' || comando === 'P') {
    ...
    // ✅ NOTIFICAR A TODOS
    if (creados.length > 0) {
      const notif = `📍 <b>Nueva posición cargada</b>\n`
        + `━━━━━━━━━━━━━━━━━━━━\n`
        + `Ubicación: <b>${ubicacionRaw}</b>\n`
        + `Productos: ${creados.length} creados\n`
        + creados.map(c => `  • ${c}`).join('\n') + '\n'
        + `👤 Por: ${nombreUsuario}`;
      await notificarATodos(notif);
    }
  ...
  if (comando === 'BORRAR') {
    ...
    if (borrados.length > 0) {
      const notif = `🗑️ <b>Productos borrados</b>\n`
        + `━━━━━━━━━━━━━━━━━━━━\n`
        + `Ubicación: <b>${ubicacion}</b>\n`
        + borrados.map(c => `  • ${c}`).join('\n') + '\n'
        + `👤 Por: ${nombreUsuario}`;
      await notificarATodos(notif);
    }
  ...
  if (comando === 'MOVER') {
    ...
    // ✅ NOTIFICAR A TODOS
    if (movidos.length > 0) {
      const notif = `🚚 <b>Movimiento de pallet</b>\n`
        + `━━━━━━━━━━━━━━━━━━━━\n`
        + `📤 Origen: <b>${origen}</b>\n`
        + `📥 Destino: <b>${destino}</b>\n`
        + `📦 Productos (${movidos.length}):\n`
        + movidos.map(c => `  • ${c}`).join('\n') + '\n'
        + `👤 Por: ${nombreUsuario}`;
      await notificarATodos(notif);
    }
  ...
  if (comando === 'ENTRADA' || comando === 'SALIDA') {
    ...
    // ✅ NOTIFICAR A TODOS
    let notif = `${icono} <b>${comando} — ${p.codigo} ${p.color}</b>\n`
      + `━━━━━━━━━━━━━━━━━━━━\n`
      + `📦 Cantidad: ${signo}${cantidad}\n`
      + `📊 Stock: ${p.stock} → <b>${nuevoStock}</b>\n`;
    if (p.ubicacion) notif += `📍 Ubicación: ${p.ubicacion}\n`;
    notif += `👤 Por: ${nombreUsuario}`;
    if (nuevoStock === 0) notif += `\n\n🚨 <b>¡SIN STOCK!</b>`;
    else if (p.minimo > 0 && nuevoStock <= p.minimo) notif += `\n\n⚠️ Stock por debajo del mínimo (${p.minimo})`;
    await notificarATodos(notif);
